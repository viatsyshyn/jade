...
exports.compile = function(str, options){
  ...
  if (options.compileDebug !== false) {
    var restorePoint = 'f' + (restorePointCounter++);
    fn = [
        'jade.debug.store("' + restorePoint + '");'
      , 'jade.debug.unshift(' + JSON.stringify(filename || '') + ', 1);'
      , 'try {'
      , parse(str, options)
      , '} catch (err) {'
      , '  jade.rethrow(err, jade.debug.peek().filename, jade.debug.peek().lineno);'
      , '} finally {'
      , '  jade.debug.restore("' + restorePoint + '");'
      , '}'
    ].join('\n');
  } else {
