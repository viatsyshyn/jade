...
Compiler.prototype = {
  ...
  visit: function(node){
    ...
    if (debug) {
      var restorePoint = 'm' + (restorePointCounter++);
      this.buf.push(
        'try {' +
        '  jade.debug.store("' + restorePoint + '");' +
        '  jade.debug.unshift(' + (node.filename ? JSON.stringify(node.filename) : '') + ',' + node.line + ');');
    }
    ...
    if (debug) {
      this.buf.push('} finally { jade.debug.restore("' + restorePoint + '"); }');
    }
  },
